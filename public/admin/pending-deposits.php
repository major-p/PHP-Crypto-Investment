...
if(isset($_GET['confirm']))
{
    ...
    $InvestQuery="INSERT INTO `investment` (`user_id`,`amount`,`plan`) VALUES  ('$uid','$amount','$plan')";
    $InvestResult=mysqli_query($con,$InvestQuery);
    $sql1="UPDATE `transactions` SET `status`='Confirmed',`updationDate`='$currentTime' WHERE `id`='$tid'";
    $result1=mysqli_query($con,$sql1);
    if($result1){  
    $updateQuery="UPDATE `users` SET `balance`='$newbal' WHERE `id`='$uid' ";   
    $result2=mysqli_query($con,$updateQuery);  
    $msg="Deposit Confirmed Successfully and User Account Updated.";
    $type = "success";
    echo '<script>
    setTimeout(function () {
      window.location ="pending-deposits.php";
  }, 3000);</script>';
    }else{
            ...
            <div class="row">
              <div class="col-md-6 grid-margin ">
                <div class="card">   
                    ...
                    <div class="card">     
                   <div class="card-body">
                    <h4 class="card-title">Investment Deposits</h4>
                    </p>
                    <div class="table-responsive">
                        <?php 
                         $query="SELECT * FROM `transactions` WHERE  `status` ='Pending' AND `coin` =''  ";
              ...
              <div class="col-md-6 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <?php 
                         $query="SELECT * FROM `transactions` WHERE `type`='Deposit' AND `status` ='Pending' AND `coin` !=''  ";
                         $result=mysqli_query($con,$query);
                         $cnt=1;
                         if(mysqli_num_rows($result)>0){
                            ?>
